...
from utils.wavelet import swt2d, swt2d_rgb
...
class PatchData(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.swt:
            lr_approx, lr_coeffs = swt2d(pair[0], wavelet=self.wavelet_func, level=self.swt_lv)
            _, hr_coeffs = swt2d(pair[1], wavelet=self.wavelet_func, level=self.swt_lv)

            lr_coeffs = np.stack(lr_coeffs)
            hr_coeffs = np.stack(hr_coeffs)
            
            lr = np.asarray(pair[0]).copy()
            hr = np.asarray(pair[1]).copy()

            if self.n_channels == 1:
                lr = np.expand_dims(lr, axis=2)
                hr = np.expand_dims(hr, axis=2)

            lr = lr.transpose((2, 0, 1)) #c,h,w
            lr = torch.from_numpy(lr).float()

            hr = hr.transpose((2, 0, 1)) #c,h,w
            hr = torch.from_numpy(hr).float()

            lr_approx = np.stack(lr_approx)

            pair = (lr_coeffs, hr_coeffs)
            pair_t = common.np2Tensor(*pair, n_channels=self.n_channels, swt=self.swt)

            return pair_t[0], pair_t[1], filename, lr, hr, lr_approx
        else : 
            pair_t = common.np2Tensor(*pair, n_channels=self.n_channels, swt=self.swt)
